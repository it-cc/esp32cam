...
#endif
  log_i("BMP: %" PRId32 "ms, %" PRIu32 "B",
        (int32_t)((fr_end - fr_start) / 1000), (uint32_t)buf_len);
  return res;
}
...
#endif
    log_i("MJPG: %" PRIu32 "B %" PRId32 "ms (%.1ffps), AVG: %" PRIu32
          "ms (%.1ffps)",
...
static int print_reg(char *p, char *end, sensor_t *s, uint16_t reg,
                     uint32_t mask)
{
