...
esp_err_t PhotoWebServer::handleIndex(httpd_req_t* req)
{
  ...
  const char* html =
      ...
      "<title>ESP32 CAM Stream</title><style>body{font-family:Arial,sans-"
      ...
      "</head><body>"
      "<h1>ESP32 CAM Stream</h1><div id=\"status\">Connecting...</div>"
      "<img id=\"live\" alt=\"stream\"><script>"
      "(function(){const statusEl=document.getElementById('status');const "
      ...
      "ws.onopen=function(){statusEl.textContent='Connected';ws.send('latest');"
      "};"
      "ws.onmessage=function(ev){if(typeof "
      "ev.data==='string'){console.log(ev.data);return;}"
